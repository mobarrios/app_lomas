...
const constants = require('./constants-e9c8ab68.js');
const helpers = require('./helpers-ddb8d8a0.js');
const frameworkDelegate = require('./framework-delegate-c37d8d5a.js');
const index = require('./index-fdf43c43.js');
...
const Nav = class {
    ...
    componentWillLoad() {
        ...
        if (this.swipeGesture === undefined) {
            ...
            this.swipeGesture = config.config.getBoolean('swipeBackEnabled', mode === 'ios');
        }
        this.ionNavWillLoad.emit();
    }
    async componentDidLoad() {
        this.rootChanged();
        this.gesture = (await new Promise(function (resolve) { resolve(require('./swipe-back-896f5186.js')); })).createSwipeBackGesture(this.el, this.canStart.bind(this), this.onStart.bind(this), this.onMove.bind(this), this.onEnd.bind(this));
        this.swipeGestureChanged();
    }
    componentDidUnload() {
        for (const view of this.views) {
            index.lifecycle(view.element, constants.LIFECYCLE_WILL_UNLOAD);
            view._destroy();
        }
        if (this.gesture) {
    ...
    postViewInit(enteringView, leavingView, ti) {
        ...
        if (destroyQueue && destroyQueue.length > 0) {
            for (const view of destroyQueue) {
                index.lifecycle(view.element, constants.LIFECYCLE_WILL_LEAVE);
                index.lifecycle(view.element, constants.LIFECYCLE_DID_LEAVE);
                index.lifecycle(view.element, constants.LIFECYCLE_WILL_UNLOAD);
            }
    ...
    async transition(enteringView, leavingView, ti) {
        ...
        const animationOpts = Object.assign({ mode, showGoBack: this.canGoBackSync(enteringView), baseEl: this.el, animationBuilder: this.animation || opts.animationBuilder || config.config.get('navAnimation'), progressCallback, animated: this.animated && config.config.getBoolean('animated', true), enteringEl,
            leavingEl }, opts);
    ...
    cleanup(activeView) {
        ...
        for (let i = views.length - 1; i >= 0; i--) {
            ...
            if (i > activeViewIndex) {
                ...
                // let's unload it
                index.lifecycle(element, constants.LIFECYCLE_WILL_UNLOAD);
                this.destroyView(view);
            }
    ...
    onEnd(shouldComplete, stepValue, dur) {
        if (this.sbAni) {
            ...
            if (!shouldComplete) {
                ...
                newStepValue += cubicBezier.getTimeGivenProgression(new cubicBezier.Point(0, 0), new cubicBezier.Point(1, 0), new cubicBezier.Point(0.68, 0.28), new cubicBezier.Point(1, 1), stepValue);
            }
            else {
                newStepValue += cubicBezier.getTimeGivenProgression(new cubicBezier.Point(0, 0), new cubicBezier.Point(0.32, 0.72), new cubicBezier.Point(0, 1), new cubicBezier.Point(1, 1), stepValue);
            }
