...
const present = async (overlay, name, iosEnterAnimation, mdEnterAnimation, opts) => {
    ...
    const animationBuilder = (overlay.enterAnimation)
        ...
        : config.config.get(name, overlay.mode === 'ios' ? iosEnterAnimation : mdEnterAnimation);
...
const dismiss = async (overlay, data, role, name, iosLeaveAnimation, mdLeaveAnimation, opts) => {
    ...
    try {
        ...
        const animationBuilder = (overlay.leaveAnimation)
            ...
            : config.config.get(name, overlay.mode === 'ios' ? iosLeaveAnimation : mdLeaveAnimation);
...
const overlayAnimation = async (overlay, animationBuilder, baseEl, opts) => {
     ...
     */
    const animation = await new Promise(function (resolve) { resolve(require('./index-09ee4a6f.js')); }).then(mod => mod.create(animationBuilder, aniRoot, opts));
    const isAnimationBuilder = animation.fill === undefined;
    if (!isAnimationBuilder) {
        animation.fill('both');
    }
    overlay.animation = animation;
    if (!overlay.animated || !config.config.getBoolean('animated', true)) {
...
const safeCall = (handler, arg) => {
    if (typeof handler === 'function') {
        const jmp = config.config.get('_zoneGate', defaultGate);
        return jmp(() => {
            try {
                return handler(arg);
            }
